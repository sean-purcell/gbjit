...
impl Ppu {
    ...
    fn render_background(&self, bus: &mut Bus) -> Scanline {
        ...
        let tmap = self.s.bg_tmap.val();
        let tdata = self.s.tile_data;

        let y = self.scroll_xy.1.wrapping_add(self.line);

        for (i, px) in line.iter_mut().enumerate() {
            let x = self.scroll_xy.0.wrapping_add(i);
            ...
            let addr = tile_addr.wrapping_add(row * 2);
            let b0 = vram.read(addr);
            let b1 = vram.read(addr + 1);

            let colour = if b0 & (1u8 << col) != 0 { 1 } else { 0 }
